...
// 画一个字符
static void setPixelDataForChar(DWORD *pixelData, const TEXTMETRICW &tm, const GLYPHMETRICS &gm, const std::vector<BYTE> &buff, LONG maxWidth, LONG maxHeight, LONG xPos, LONG yPos, LONG demarcation, const std::vector<DWORD> &colorPast, const std::vector<DWORD> &colorFuture) {
...
static inline DWORD convertRGBtoBGR(COLORREF c) {
    return ((c & 0xFF) << 16) | (c & 0xFF00) | ((c & 0xFF0000) >> 16);
}

static std::vector<DWORD> gradientColor(const std::vector<COLORREF> &colors, LONG height) {
...
static RECT setPixelDataForSentence(DWORD *pixelData, LONG maxWidth, LONG maxHeight, unsigned align,
    ...
    for (size_t i = 0, cnt = details.size(); i < cnt; ++i) {
        ...
        for (size_t k = 0, cnt = lwd.text_indices.size(); k < cnt; ++k) {
            ...
            setPixelDataForChar(pixelData, tm1, gm, buff, maxWidth, maxHeight, lwd.text_pos + offset, yPosText, demarcation + xPos, colorPast1, colorFuture1);
        ...
        for (size_t k = 0, cnt = lwd.phonetic_indices.size(); k < cnt; ++k) {
            ...
            setPixelDataForChar(pixelData, tm2, gm, buff, maxWidth, maxHeight, lwd.phonetic_pos + offset, yPosPhonetic, demarcation + xPos, colorPast2, colorFuture2);
...
static void setPixelDataForCountingdown(DWORD *pixelData, LONG maxWidth, LONG maxHeight, LONG yPos, HDC hdc, WCHAR ch, int cnt, const TEXTMETRICW &tm1, const std::vector<DWORD> &color) {
    ...
    LONG totalWidth = 0;
    for (int i = 0; i < cnt; ++i) {
        setPixelDataForChar(pixelData, tm1, gm, buff, maxWidth, maxHeight, totalWidth, yPos, INT_MAX, color, color);
...
void DesktopWindow::drawSentence(const DrawInfo *info1, const DrawInfo *info2) {
    ...
    std::vector<DWORD> colorPast1 = gradientColor(_colorPast, tm1.tmHeight);
    std::vector<DWORD> colorFuture1 = gradientColor(_colorFuture, tm1.tmHeight);
    std::vector<DWORD> colorPast2 = gradientColor(_colorPast, tm2.tmHeight);
    std::vector<DWORD> colorFuture2 = gradientColor(_colorFuture, tm2.tmHeight);
...
void DesktopWindow::drawInfo(const std::wstring &text) {
    ...
    do {
        ...
        for (size_t i = 0, cnt = textIndices.size(); i < cnt; ++i) {
            ...
            setPixelDataForChar(pixelData, tm1, gm, buff, maxWidth, maxHeight, xPos + offset, yPos, INT_MAX, colorPast1, colorPast1);
